 ...
 * @author		m_richa
 * @date		05/10/22
   @version 	1.0.F Added s_task_get_info function 
   			1.0.E Added s_arg to task 1.0.D	YIELD() ) macro added 
				1.0.C Added d d s_task_b_delay 
				...
				1.0.5 Modified for IPGLB code style Added mailbox s_task 3-level priority added
...
bool s_task_get_info(s_task_handle_t s_task_hndl, s_task_info_t *s_task_info)
{
	...
	if(node != NULL)
	{
		s_task_info->s_active = node->active;
		s_task_info ->s_priority = node->s_priority;
		s_task_info->s_ticks = node->s_ticks;
		return true;
	}
